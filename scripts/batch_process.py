...
def main():
    ...
    parser.add_argument('--gt_dir', default=None, help='Ground truth segmentation folder for subset filtering')
    ...
    if args.gt_dir:
        gt_files = sorted(glob(os.path.join(args.gt_dir, '*.*')))
        gt_stems = set(Path(fname).stem for fname in gt_files)
        images = [p for p in images if Path(p).stem in gt_stems]
